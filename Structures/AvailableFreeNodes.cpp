...
void AvailableFreeNodes::addNodeToStruct(GraphNode *node, FreeNodeStructure *structure) {
    ...
    if (node->isBlossom) {
        // If we have a blossom we add each vertex to the structure dictionary.
        ...
        for (Vertex vertex: blossom->vertices_in_blossom) {
            ...
            structure->addVertexToStruct(vertex, node);
        }
    } else {
        ...
        structure->addVertexToStruct(node->vertex_id, node);
    }
    //
    ...
    for (GraphNode* child : node->children) {
        addNodeToStruct(child, structure);
    }
}
...
void AvailableFreeNodes::removeNodeFromStruct(GraphNode* node, FreeNodeStructure* structure) {
    if (node->isBlossom) {
        ...
        GraphBlossom* blossom = dynamic_cast<GraphBlossom*>(node);
        for (Vertex vertex : blossom->vertices_in_blossom) {
            structure->vertex_to_graph_node.erase(vertex);
            vertex_to_struct.erase(vertex);
        }
    } else {
        structure->vertex_to_graph_node.erase(node->vertex_id);
        vertex_to_struct.erase(node->vertex_id);
    }
